...
#pragma unroll 1
    for (size_t i{0}; i < NUM_ITERS; ++i)
    {
        ...
        if constexpr (D_SIZE == 4 && A_SIZE == 2 && B_SIZE == 1 && C_SIZE == 4)
        {
...
int main()
{
    ...
    std::cout << "========================================" << std::endl;
    std::cout << "CUTLASS SM75 MMA Atom Benchmark" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    {
        std::cerr << "Error: This benchmark requires SM75 or later (Turing "
                     "architecture)"
                  << std::endl;
        return 1;
    }
    ...
    std::cout << "Benchmarking SM75 MMA Atoms:" << std::endl;
    ...
    BENCHMARK_MMA(SM75_16x8x8_F32F16F16F32_TN, 16, 8, 8, NUM_ITERS);

    // INT32 Output (S8xS8 Input) MMA Atom (8x8x16)
    std::cout << std::endl << "=== INT32 Output (S8×S8 Input) ===" << std::endl;
    BENCHMARK_MMA(SM75_8x8x16_S32S8S8S32_TN, 8, 8, 16, NUM_ITERS);
